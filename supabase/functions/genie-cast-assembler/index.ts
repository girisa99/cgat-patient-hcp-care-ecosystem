...
serve(async (req) => {
  ...
  let outerCastProjectId: string | null = null;
  try {
    const { 
      language = 'en', 
      quality = 'production', 
    ...
    if (mode === 'submit-timeline' && timeline && timeline.scenes && timeline.scenes.length > 0) {
      console.log(`🎬 SUBMIT-TIMELINE mode — forwarding pre-built timeline with ${timeline.scenes.length} scenes`);
      const totalDuration = timeline.scenes.reduce((sum: number, s: any) => sum + (s.duration || 0), 0);
      console.log(`   Total duration: ${totalDuration}s, payload size: ~${Math.round(JSON.stringify(timeline).length / 1024)}KB`);
    ...
    // (FALLBACK — new code uses submit-timeline above)
    // ═══════════════════════════════════════════════════════════════════════
