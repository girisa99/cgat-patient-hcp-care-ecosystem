-- Add missing major pharmaceutical manufacturers (skip if already exists)
DO $$
BEGIN
...
EXCEPTION WHEN unique_violation THEN
  -- Manufacturers already exist, skip
  NULL;
END $$;
...
WHERE NOT EXISTS (
  SELECT 1 FROM products p WHERE p.name = v.product_name
);
