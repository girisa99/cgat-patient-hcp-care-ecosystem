...
INSERT INTO workflow_node_categories (name, description, icon, color, is_active) VALUES
('Document Loaders', 'File processing and data ingestion nodes', 'FileText', '#3b82f6', true),
('GenAI & LLM', 'Large Language Models and AI processing', 'Brain', '#8b5cf6', true),
('Vector Stores', 'Vector databases and similarity search', 'Database', '#06b6d4', true),
('Healthcare & Compliance', 'Healthcare-specific and compliance nodes', 'Heart', '#ef4444', true),
('Tools & Utilities', 'General purpose utility and integration tools', 'Wrench', '#f59e0b', true),
('Code & Deployment', 'Development and deployment automation', 'Code', '#10b981', true),
('Voice Configuration', 'Speech and audio processing capabilities', 'Mic', '#f97316', true),
('Channel Deployment', 'Multi-channel bot deployment options', 'MessageSquare', '#6366f1', true),
('Agent Flows', 'Multi-agent orchestration and collaboration', 'Users', '#ec4899', true),
('Human Loop', 'Human-in-the-loop workflow components', 'User', '#84cc16', true),
('Cache & Memory', 'Memory management and caching solutions', 'HardDrive', '#06b6d4', true),
('MCP Protocol', 'Model Context Protocol server integrations', 'Plug', '#8b5cf6', true);

-- Insert consolidated 84 node types
INSERT INTO workflow_node_types (key, name, description, category_id, configuration_schema, is_active) VALUES
...
-- Update node counts in categories
UPDATE workflow_node_categories SET node_count = (
  SELECT COUNT(*) FROM workflow_node_types WHERE category_id = workflow_node_categories.id
);

-- Create indices for performance
CREATE INDEX IF NOT EXISTS idx_workflow_node_types_category ON workflow_node_types(category_id);
CREATE INDEX IF NOT EXISTS idx_workflow_node_types_key ON workflow_node_types(key);
CREATE INDEX IF NOT EXISTS idx_workflow_node_types_active ON workflow_node_types(is_active);

-- Log the consolidation
INSERT INTO active_issues (issue_type, issue_message, issue_source, issue_severity, category, status) VALUES
('consolidation_complete', 'Successfully consolidated workflow nodes from 272 duplicated nodes to 84 unique nodes across 12 categories', 'node_consolidation', 'info', 'system', 'resolved');