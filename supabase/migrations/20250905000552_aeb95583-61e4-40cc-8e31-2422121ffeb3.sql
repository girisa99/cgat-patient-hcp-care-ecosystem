-- Seed default deployment environments (simple insert)
...
FROM auth.users 
LIMIT 1
WHERE NOT EXISTS (SELECT 1 FROM public.deployment_environments WHERE name = 'Development');
...
FROM auth.users 
LIMIT 1
WHERE NOT EXISTS (SELECT 1 FROM public.deployment_environments WHERE name = 'Staging');
...
FROM auth.users 
LIMIT 1
WHERE NOT EXISTS (SELECT 1 FROM public.deployment_environments WHERE name = 'Production');