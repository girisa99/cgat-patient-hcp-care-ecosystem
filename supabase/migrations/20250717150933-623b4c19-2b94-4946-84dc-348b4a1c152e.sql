-- First create the supporting tables with proper structure
...
-- Create API categories reference table (without constraint dependencies)
...
ALTER TABLE public.api_categories ADD CONSTRAINT fk_parent_category
  FOREIGN KEY (parent_category_id) REFERENCES public.api_categories(id);
...
('Communication', 'communication', 'Messaging, notifications, and communication', 'MessageSquare', 'cyan'),
('Documentation', 'documentation', 'API documentation and specification management', 'FileText', 'yellow'),
('Compliance & Security', 'compliance', 'Regulatory compliance and security services', 'Shield', 'pink')
ON CONFLICT (category_name) DO NOTHING;

-- Now insert the APIs with proper category values (matching existing schema)
...
-- Business Layer APIs (using 'healthcare' as primary category)
