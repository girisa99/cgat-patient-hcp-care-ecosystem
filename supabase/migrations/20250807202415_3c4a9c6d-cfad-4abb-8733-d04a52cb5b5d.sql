...
-- Get the demo user role ID for assignments
DO $$
DECLARE
    demo_role_id uuid;
...
-- Demo users can view but not modify critical tables
CREATE POLICY IF NOT EXISTS "demo_users_read_profiles" 
ON public.profiles 
FOR SELECT 
TO authenticated
USING (
...
CREATE POLICY IF NOT EXISTS "demo_users_read_facilities" 
...
-- Demo users CANNOT modify critical data
CREATE POLICY IF NOT EXISTS "demo_users_no_write_profiles" 
ON public.profiles 
FOR ALL
TO authenticated
USING (
...
-- Function to check if user is demo user
