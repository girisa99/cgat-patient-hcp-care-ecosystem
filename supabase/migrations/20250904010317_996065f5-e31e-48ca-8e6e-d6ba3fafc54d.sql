-- PHASE 2 (Fixed): Migrate workflow_builder_nodes to workflow_node_types
-- Handle duplicate type_key conflicts and skip existing nodes
...
DECLARE
    ...
    nodes_skipped INTEGER := 0;
    generated_type_key TEXT;
BEGIN
    ...
    LOOP
        ...
        -- Generate type_key and check for conflicts
        generated_type_key := LOWER(REPLACE(REPLACE(node_mapping.label, ' ', '_'), '/', '_'));
        
        -- If type_key already exists, append suffix or skip
        IF EXISTS (SELECT 1 FROM workflow_node_types WHERE type_key = generated_type_key) THEN
            -- Try with category prefix
            generated_type_key := node_mapping.category || '_' || generated_type_key;
            
            -- If still exists, skip this node
            IF EXISTS (SELECT 1 FROM workflow_node_types WHERE type_key = generated_type_key) THEN
                RAISE NOTICE 'Skipping duplicate node: % (type_key: %)', node_mapping.label, generated_type_key;
                nodes_skipped := nodes_skipped + 1;
                CONTINUE;
            END IF;
        END IF;
        ...
        INSERT INTO workflow_node_types (
            ...
            COALESCE(node_mapping.configuration, '{}'::jsonb),
            '{}'::jsonb, -- input_schema
            '{}'::jsonb, -- output_schema
            '[]'::jsonb, -- capabilities
            '{}'::jsonb, -- requirements
            1000 + nodes_migrated, -- order_index (start high to avoid conflicts)
            true, -- is_active
            NULL, -- created_by
            now(),
            now()
    ...
    RAISE NOTICE 'Node migration completed. Migrated: %, Skipped: %', nodes_migrated, nodes_skipped;
