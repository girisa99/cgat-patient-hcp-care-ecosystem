...
BEGIN
  ...
  IF TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND OLD.test_coverage_status IS DISTINCT FROM NEW.test_coverage_status) THEN
    -- Schedule background sync (using notify for async processing)
    PERFORM pg_notify('role_based_test_sync', NEW.id::text);
  END IF;
