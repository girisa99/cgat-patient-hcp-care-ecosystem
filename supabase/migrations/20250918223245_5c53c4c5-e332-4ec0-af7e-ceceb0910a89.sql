...
DO $$
BEGIN
  -- Drop the CHECK constraint if it exists (name inferred from error)
  IF EXISTS (
...
-- patient_enrollments policies
CREATE POLICY IF NOT EXISTS "patient_enrollments_insert_own"
ON public.patient_enrollments
FOR INSERT
WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "patient_enrollments_select_own"
ON public.patient_enrollments
FOR SELECT
USING (auth.uid() = user_id OR is_admin_user_safe(auth.uid()));

CREATE POLICY IF NOT EXISTS "patient_enrollments_update_own"
ON public.patient_enrollments
FOR UPDATE
USING (auth.uid() = user_id OR is_admin_user_safe(auth.uid()));

CREATE POLICY IF NOT EXISTS "patient_enrollments_delete_own"
ON public.patient_enrollments
FOR DELETE
USING (auth.uid() = user_id OR is_admin_user_safe(auth.uid()));

-- enrollment_consent policies - link via parent enrollment record
CREATE POLICY IF NOT EXISTS "enrollment_consent_insert_via_parent"
...
CREATE POLICY IF NOT EXISTS "enrollment_consent_select_via_parent"
ON public.enrollment_consent
FOR SELECT
USING (
  EXISTS (
...
CREATE POLICY IF NOT EXISTS "enrollment_consent_update_via_parent"
ON public.enrollment_consent
FOR UPDATE
USING (
  EXISTS (
...
CREATE POLICY IF NOT EXISTS "enrollment_consent_delete_via_parent"
ON public.enrollment_consent
FOR DELETE
USING (
  EXISTS (
