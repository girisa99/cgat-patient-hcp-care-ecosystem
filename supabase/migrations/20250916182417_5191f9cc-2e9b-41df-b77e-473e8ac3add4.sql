...
-- 1. Fix the three security definer views by removing SECURITY DEFINER where not needed
-- The views causing issues are likely the performance monitoring views

-- Drop and recreate views without SECURITY DEFINER if they exist
DROP VIEW IF EXISTS performance_stats_summary CASCADE;
DROP VIEW IF EXISTS database_health_overview CASCADE;

-- Recreate performance monitoring views without SECURITY DEFINER
CREATE OR REPLACE VIEW performance_stats_summary AS
SELECT 
...
UNION ALL
SELECT 
  'largest_table' as metric_name,
  schemaname||'.'||tablename as metric_value,
  'Largest Table' as description
FROM pg_tables 
JOIN (
  SELECT schemaname, tablename, 
         ROW_NUMBER() OVER (ORDER BY pg_total_relation_size(schemaname||'.'||tablename) DESC) as rn
  FROM pg_tables 
  WHERE schemaname = 'public'
) ranked ON pg_tables.schemaname = ranked.schemaname AND pg_tables.tablename = ranked.tablename
WHERE ranked.rn = 1;

-- 2. Fix function search paths for the functions we just created
