...
INSERT INTO public.commercial_products (product_id, launch_date, market_regions, reimbursement_status, competitive_landscape, patent_expiry, market_size_estimates, pricing_strategy, distribution_channels, real_world_evidence, is_active)
SELECT 
  p.id,
  p.approval_date,
  ARRAY['US', 'EU', 'Japan'],
  CASE 
    WHEN p.name LIKE '%Amtagvi%' OR p.name LIKE '%Casgevy%' OR p.name LIKE '%Zolgensma%' THEN 'limited'
    WHEN p.name LIKE '%Aduhelm%' THEN 'controversial'
    ELSE 'covered'
  END::reimbursement_status,
  '{"market_position": "leading", "key_competitors": 2}'::jsonb,
  (p.approval_date + INTERVAL '12 years')::date,
  '{"peak_sales": "1B+", "patient_population": "orphan"}'::jsonb,
  '{"strategy": "premium_pricing", "access_programs": true}'::jsonb,
  ARRAY['specialty_pharmacy', 'hospital_direct'],
  '{"studies_completed": 5, "ongoing_studies": 3}'::jsonb,
  true
FROM products p
WHERE p.name IN (
