...
-- Clean up existing enrollment tables and add management features

-- 1. Clean up all enrollment-related tables in correct order (respecting foreign keys)

-- Delete all enrollment documents
DELETE FROM enrollment_documents WHERE enrollment_id IN (SELECT id FROM patient_enrollments);

-- Delete all enrollment treatment plans
DELETE FROM enrollment_treatment_plan WHERE enrollment_id IN (SELECT id FROM patient_enrollments);

-- Delete all enrollment consent records
DELETE FROM enrollment_consent WHERE enrollment_id IN (SELECT id FROM patient_enrollments);
...
-- Delete all enrollment collaborations
DELETE FROM enrollment_collaborations WHERE enrollment_id IN (SELECT id FROM patient_enrollments);

-- Delete all enrollment instances
DELETE FROM enrollment_instances WHERE enrollment_id IN (SELECT id FROM patient_enrollments);

-- Delete all real-time sync records
DELETE FROM enrollment_real_time_sync WHERE enrollment_id IN (SELECT id FROM patient_enrollments);

-- Delete all WhatsApp enrollment sessions
