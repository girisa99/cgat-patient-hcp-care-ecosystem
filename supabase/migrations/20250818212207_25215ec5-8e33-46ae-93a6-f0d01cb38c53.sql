-- Secure Healthcare Data Tables
-- Restrict clinical_trials, commercial_products, manufacturers, products to authorized roles only

-- Clinical trials table - sensitive healthcare research data
...
-- Commercial products table - business sensitive data
...
-- Manufacturers table - business intelligence data
...
-- Products table - commercial product information
...
-- Treatment center onboarding - patient/business sensitive data
...
FOR SELECT 
USING ((requested_by = auth.uid()) OR is_admin_user_safe(auth.uid()) OR is_demo_user(auth.uid()));

CREATE POLICY "Users can manage their own onboarding records" 
ON public.treatment_center_onboarding 
FOR ALL 
USING ((requested_by = auth.uid()) OR is_admin_user_safe(auth.uid()))
WITH CHECK ((requested_by = auth.uid()) OR is_admin_user_safe(auth.uid()));

-- System Configuration Tables - restrict to authenticated users with proper roles
-- Use cases table
