...
-- Drop any existing demo user policies first
DROP POLICY IF EXISTS "demo_users_read_profiles" ON public.profiles;
DROP POLICY IF EXISTS "demo_users_read_facilities" ON public.facilities;
DROP POLICY IF EXISTS "demo_users_no_write_profiles" ON public.profiles;

-- Create enhanced RLS policies for demo users (read-only access)
-- Demo users can view profiles and facilities
CREATE POLICY "demo_users_read_profiles" 
ON public.profiles 
FOR SELECT 
TO authenticated
USING (public.is_demo_user(auth.uid()));

CREATE POLICY "demo_users_read_facilities" 
ON public.facilities 
FOR SELECT 
TO authenticated
USING (public.is_demo_user(auth.uid()));

-- Demo users CANNOT modify critical data (INSERT, UPDATE, DELETE blocked)
CREATE POLICY "demo_users_no_write_profiles" 
ON public.profiles 
FOR ALL
