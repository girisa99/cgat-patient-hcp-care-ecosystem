-- Enforce duplicate checks and admin CRUD policies for key operational tables
-- Uses SECURITY DEFINER helper and safe checks. Idempotent: only applies to existing tables.

-- 0) Helper to check table existence
...
-- 4) Profiles (patients/users) duplicate checks
DO $$ BEGIN
...
-- 5) Treatment Center Onboarding duplicate checks
DO $$ BEGIN
...
-- 6) Role/Module/User assignment deduplication (role_module_assignments, user_module_assignments, user_roles)
DO $$ BEGIN
...
DO $$ BEGIN
  IF public._table_exists('user_roles') THEN
    ...
    -- Admin ALL is already set via previous migrations; add safe policy to ensure
