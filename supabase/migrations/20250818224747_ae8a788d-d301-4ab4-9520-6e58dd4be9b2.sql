-- PHASE 1: Critical Security Fixes - Preserve All Existing Functionality (CORRECTED FUNCTION)
...
-- Function to log sensitive data access (corrected version)
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE information_schema.tables.table_name = 'audit_logs') 
    ...
    INSERT INTO audit_logs (
      ...
      auth.uid(),
      p_operation_type || ' on ' || p_table_name,
      p_table_name,
      p_record_id,
      jsonb_build_object(
        'timestamp', now(),
