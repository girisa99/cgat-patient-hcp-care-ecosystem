-- Add missing performance-critical indexes (without CONCURRENTLY for migration)
...
CREATE INDEX IF NOT EXISTS idx_active_issues_severity_created 
ON active_issues(issue_severity, created_at DESC);

-- Optimize JSONB queries on agent configurations
CREATE INDEX IF NOT EXISTS idx_agents_configuration_gin 
