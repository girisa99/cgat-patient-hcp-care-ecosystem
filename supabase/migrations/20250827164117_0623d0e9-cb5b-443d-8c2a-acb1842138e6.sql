-- Safe Migration: Optimize User Roles Structure (Fixed)
-- This migration ensures data integrity and performance without breaking functionality

-- Step 1: Remove any rows with NULL user_id or role_id (these would be invalid anyway)
DELETE FROM public.user_roles 
WHERE user_id IS NULL OR role_id IS NULL;

-- Step 2: Remove any duplicate assignments (keep the oldest one)
DELETE FROM public.user_roles 
WHERE id NOT IN (
...
-- Step 5: Add performance indexes (without CONCURRENTLY for transaction compatibility)
...
-- Step 6: Update the validation trigger to be more robust
...
-- Step 7: Ensure the trigger is attached
...
-- Step 8: Add helpful utility function for role checking (performance optimization)
