...
export const useEnrollmentPatients = () => {
  ...
  // Fetch patients from enrollment tables
  const { data: patients = [], isLoading, error } = useQuery({
    queryKey: ['enrollment-patients'],
    queryFn: async (): Promise<EnrollmentPatient[]> => {
      console.log('🏥 Fetching patients from enrollment tables...');
      
      // Fetch patient enrollments with optional patient info
      ...
      if (error) {
        console.error('❌ Error fetching enrollment patients:', error);
        throw error;
      }
      ...
      const transformedData = data.map(enrollment => ({
        ...
        enrollment_patient_info: undefined // Remove the nested object
      }));

      console.log('✅ Successfully fetched enrollment patients:', transformedData.length);
