...
export const useApiKeyMonitor = () => {
  ...
  useEffect(() => {
    ...
    const processNewApiKeys = async () => {
      ...
      for (const apiKey of newApiKeys) {
        console.log(`🔑 New API key detected: ${apiKey.name} (${apiKey.type})`);
        ...
        try {
          ...
          console.log(`✅ Auto-integration triggered for API key: ${apiKey.name}`);
          ...
          console.error(`❌ Failed to trigger auto-integration for ${apiKey.name}:`, error);
          toast({
  ...
  const registerApiKeyAsExternalApi = async (apiKey: ApiKeyWithIntegration, apiType: 'internal' | 'external' | 'consumed') => {
    try {
      ...
      console.log(`📝 Registered API key as ${apiType} API:`, data);
      return data;
    } catch (error) {
