...
const priorityConfig = {
  p0: { label: 'P0 - Core', color: 'bg-emerald-600', icon: Zap, description: 'Essential MVP features' },
  p1: { label: 'P1 - Enhanced', color: 'bg-blue-600', icon: Layers, description: 'Important enhancements' },
  p2: { label: 'P2 - Advanced', color: 'bg-amber-600', icon: TrendingUp, description: 'Advanced capabilities' },
  p3: { label: 'P3 - Differentiator', color: 'bg-purple-600', icon: Target, description: 'Competitive features' },
  p4: { label: 'P4 - Future', color: 'bg-slate-600', icon: Shield, description: 'Future roadmap' },
};
...
export const GenieStudioScenarioMapDiagram = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'implemented':
        return <Badge className="bg-green-600 text-xs">✓ Done</Badge>;
      case 'partial':
        return <Badge className="bg-yellow-600 text-xs">◐ Partial</Badge>;
      default:
        return <Badge className="bg-slate-600 text-xs">○ Planned</Badge>;
    }
  };
        ...
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-2xl text-white">Scenario Priority Map</CardTitle>
                <p className="text-amber-300 text-sm">60 Scenarios across P0-P4 priorities</p>
              </div>
            </div>
              ...
              <CardContent className="pt-6">
                <svg viewBox="0 0 1200 450" className="w-full h-auto">
                  ...
                  {Object.entries(priorityConfig).map(([key, config], index) => {
                    ...
                    return (
                      <g key={key} transform={`translate(${x}, 100)`}>
                        <rect width="200" height="300" rx="12" fill="#1e293b" stroke="#475569" strokeWidth="1" />
                        <rect width="200" height="50" rx="12" fill={config.color.replace('bg-', '#').replace('-600', '')} className={config.color} />
                        
                        {/* Need to map color names to hex */}
                        <rect width="200" height="50" rx="12" 
                          fill={
                            key === 'p0' ? '#059669' : 
                            key === 'p1' ? '#2563eb' : 
                            key === 'p2' ? '#d97706' : 
                            key === 'p3' ? '#7c3aed' : '#475569'
                          } 
                        />
                        
                        <text x="100" y="32" textAnchor="middle" fill="#fff" fontSize="14" fontWeight="bold">
                          {config.label}
                        </text>
                        ...
                        {/* Progress bar */}
                        <rect x="20" y="95" width="160" height="12" rx="6" fill="#374151" />
                        <rect x="20" y="95" width={160 * (stats.percent / 100)} height="12" rx="6" 
                          fill={
                            key === 'p0' ? '#10b981' : 
                            key === 'p1' ? '#3b82f6' : 
                            key === 'p2' ? '#f59e0b' : 
                            key === 'p3' ? '#8b5cf6' : '#6b7280'
                          }
                        />
                        <text x="100" y="123" textAnchor="middle" fill="#e2e8f0" fontSize="11">
                          {stats.percent}% Complete
                        </text>
                        
                        {/* Status breakdown */}
                        <g transform="translate(20, 140)">
                          <rect width="50" height="20" rx="4" fill="#22c55e" />
                          <text x="25" y="14" textAnchor="middle" fill="#fff" fontSize="10">{stats.implemented}</text>
                          <text x="70" y="14" fill="#22c55e" fontSize="9">Done</text>
                          
                          <rect y="30" width="50" height="20" rx="4" fill="#eab308" />
                          <text x="25" y="44" textAnchor="middle" fill="#fff" fontSize="10">{stats.partial}</text>
                          <text x="70" y="44" fill="#eab308" fontSize="9">Partial</text>
                          
                          <rect y="60" width="50" height="20" rx="4" fill="#6b7280" />
                          <text x="25" y="74" textAnchor="middle" fill="#fff" fontSize="10">{stats.planned}</text>
                          <text x="70" y="74" fill="#6b7280" fontSize="9">Planned</text>
                        </g>
                        
                        {/* Category highlights */}
                        <text x="20" y="240" fill="#94a3b8" fontSize="9" fontWeight="bold">Top Categories:</text>
                        {scenarios[key]?.slice(0, 3).map((s, i) => (
                          <text key={s.id} x="20" y={258 + i * 14} fill="#64748b" fontSize="8">
                            • {s.category}
                          </text>
                        ))}
                      </g>
                  ...
                  <g transform="translate(60, 420)">
                    <text x="0" y="0" fill="#94a3b8" fontSize="10">Legend:</text>
                    <rect x="60" y="-10" width="12" height="12" rx="2" fill="#22c55e" />
                    <text x="78" y="0" fill="#94a3b8" fontSize="10">Implemented</text>
                    <rect x="160" y="-10" width="12" height="12" rx="2" fill="#eab308" />
                    <text x="178" y="0" fill="#94a3b8" fontSize="10">Partial</text>
                    <rect x="240" y="-10" width="12" height="12" rx="2" fill="#6b7280" />
                    <text x="258" y="0" fill="#94a3b8" fontSize="10">Planned</text>
                  </g>
                </svg>
              </CardContent>
            </Card>
                ...
                return (
                  <Card key={key} className="bg-slate-800/50 border-slate-600">
                    <CardContent className="pt-4">
                      ...
                      <p className="text-xs text-slate-500">{stats.implemented}/{stats.total} complete</p>
                    </CardContent>
                  </Card>
                );
              })}
                        ...
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              <span className="text-slate-500 text-xs">#{scenario.id}</span>
                              <span className="text-white font-medium text-sm">{scenario.name}</span>
                            </div>
                            <p className="text-slate-400 text-xs mt-1">{scenario.description}</p>
