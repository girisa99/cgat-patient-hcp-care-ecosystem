...
import { Users, UserPlus, Settings, RefreshCw, Search, Shield, Building2 } from 'lucide-react';
...
export const UnifiedUserManagementComponent: React.FC = () => {
  ...
  console.log('🎯 Unified User Management Component - FIXED INTERFACES');
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            Unified User Management ({users.length} users)
          </div>
          <div className="flex items-center gap-2">
            <Button
      ...
      <CardContent>
        <div className="space-y-4">
          ...
          {(isCreatingUser || isAssigningRole || isRemovingRole || isAssigningFacility || isDeactivating) && (
            <div className="mt-4 p-4 bg-blue-50 rounded-lg">
              <div className="flex items-center gap-2 text-blue-600">
                <RefreshCw className="h-4 w-4 animate-spin" />
                <span className="text-sm">
                  {isCreatingUser && 'Creating user...'}
                  {isAssigningRole && 'Assigning role...'}
                  {isRemovingRole && 'Removing role...'}
                  {isAssigningFacility && 'Assigning facility...'}
                  {isDeactivating && 'Deactivating user...'}
                </span>
              </div>
            </div>
          )}
