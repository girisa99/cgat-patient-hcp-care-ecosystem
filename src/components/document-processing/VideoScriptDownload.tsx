...
const VideoScriptDownload = () => {
  const handleDownload = () => {
    ...
    toast.success('Video script downloaded successfully!');
  };
    ...
    <Card className="w-full max-w-2xl mx-auto">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Video Script - Document Processing Walkthrough
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <p className="text-muted-foreground">
          Complete 8-9 minute video script for the AI-Powered Document Processing platform walkthrough. 
          Includes timestamps, screen directions, narration, YouTube description template, and recording tips.
        </p>
        
        <div className="flex flex-col gap-2 text-sm text-muted-foreground">
          <div className="flex items-center gap-2">
            <FileText className="h-4 w-4" />
            <span>Sections: Intro, Pipeline Overview, Prescription Demo, Insurance Cards, Medical Imaging, RCM Analysis, MCP Export, Sub-Agents, History, Closing</span>
          </div>
        </div>
...
export default VideoScriptDownload;
