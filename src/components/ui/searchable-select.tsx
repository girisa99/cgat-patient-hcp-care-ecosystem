...
export const SearchableSelect: React.FC<SearchableSelectProps> = ({
  ...
  return (
    <div ref={dropdownRef} className={cn("relative w-full", className)}>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-1 bg-popover border rounded-md shadow-lg z-[9999] max-h-80 overflow-hidden">
          {/* Search Input */}
          <div className="p-2 border-b bg-background sticky top-0">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-7 pr-2 py-1.5 text-xs border rounded focus:outline-none focus:ring-2 focus:ring-ring bg-background"
              />
            </div>
          </div>
                ...
                {categoryOptions.map((option) => {
                  ...
                  return (
                    <div
                      key={option.value}
                      className={cn(
                        "flex items-center gap-2 p-2 cursor-pointer rounded-sm text-xs",
