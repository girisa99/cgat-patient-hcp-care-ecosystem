...
export const MultiSelectDropdown: React.FC<MultiSelectDropdownProps> = ({
  ...
  // Calculate dropdown position using fixed positioning for portal - memoized with dependencies
  ...
  // Update position on scroll/resize - just call updateDropdownPosition
  useEffect(() => {
    if (!isOpen) return;
  ...
  const dropdownContent = isOpen ? createPortal(
    ...
    >
      ...
      <div 
        className="flex-1 overflow-y-auto bg-popover"
        style={{ 
          overflowY: 'auto',
      ...
      {maxSelections && (
        <div className="p-2 border-t border-border text-xs text-muted-foreground text-center bg-muted/30 flex-shrink-0">
