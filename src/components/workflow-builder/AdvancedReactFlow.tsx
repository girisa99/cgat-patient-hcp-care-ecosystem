...
const ConsolidatedAdvancedReactFlowContent: React.FC<ConsolidatedAdvancedReactFlowProps> = ({
  ...
  const handleConfigureNode = useCallback((nodeId: string, nodeType: string, category: string) => {
    setConfigNodeInfo({ nodeId, nodeType, category });
        ...
        <div className="absolute inset-0 pointer-events-none z-20">
          <div className="absolute top-4 left-1/2 transform -translate-x-1/2 pointer-events-auto">
            <Alert className="bg-yellow-50 border-yellow-200">
              ...
              <AlertDescription>
                Test Mode Active - Click nodes to test execution
                <Button 
                  size="sm" 
                  variant="outline" 
                  className="ml-2"
        ...
        <div className="fixed top-20 right-4 w-96 max-h-[calc(100vh-120px)] border bg-background rounded-lg shadow-lg z-40">
          <div className="p-3 border-b flex items-center justify-between">
            <h3 className="font-semibold text-sm">
              Configure: {(() => {
                const n = getNodes().find(node => node.id === configNodeInfo.nodeId);
                const d = n?.data;
