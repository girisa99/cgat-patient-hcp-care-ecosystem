...
const RESUME_CONTENT = `# Genie AI - Mind to Media Production Suite
...
| **Google** | Gemini 2.5 Pro/Flash, Gemini 2.5 Flash, Gemini Vision |
...
- **15** AI Providers integrated (Google Vertex, Claude, OpenAI, Azure, Alibaba, DeepSeek, Deepgram, ElevenLabs, DeepL, Meshy, ModelsLab, Replicate, JSON2Video, Gemini, Supabase)
...
export const GenieResumeDownload: React.FC = () => {
  const [copied, setCopied] = useState(false);

  const downloadAsMarkdown = () => {
    const blob = new Blob([RESUME_CONTENT], { type: 'text/markdown;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = 'Genie_AI_Resume_Writeup.md';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
    toast.success('Resume document downloaded!');
  };
  ...
  const downloadAsTxt = () => {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = 'Genie_AI_Resume_Writeup.txt';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
    toast.success('Resume document downloaded as TXT!');
  };

  const copyToClipboard = async () => {
    try {
      await navigator.clipboard.writeText(RESUME_CONTENT);
      setCopied(true);
    ...
    <Card className="border-primary/20 bg-gradient-to-br from-background to-muted/20">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Genie AI Resume Write-up
        </CardTitle>
        <CardDescription>
          Comprehensive technical resume covering Universal AI, Custom Agents, A2A Protocol, MCP SDK, and Mind to Media Production Suite
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <Button onClick={downloadAsMarkdown} className="w-full">
        ...
        <div className="text-sm text-muted-foreground space-y-1">
          ...
          <ul className="list-disc list-inside space-y-1 ml-2">
            <li>Executive Summary & Unique Features</li>
            <li>Universal AI Processor & Multi-Provider Architecture</li>
            <li>12+ Custom Agents (Healthcare, Media, Document Processing)</li>
            <li>A2A Protocol & Swarm Intelligence Implementation</li>
            <li>MCP SDK Integration Details</li>
            <li>60+ Edge Functions Registry</li>
            <li>Ready-to-use Resume Bullet Points</li>
            <li>Metrics & Impact Statistics</li>
          </ul>
        </div>
      </CardContent>
    </Card>
...
export default GenieResumeDownload;
