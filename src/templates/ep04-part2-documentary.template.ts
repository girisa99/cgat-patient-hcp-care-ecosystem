...
export function buildEP04Part2Template(): ProjectTemplate {
  ...
  const bookends: ProjectTemplate['bookends'] = EP04_PART2_STORYBOOK_BOOKENDS ? {
    opening: {
      duration: (EP04_PART2_STORYBOOK_BOOKENDS.opening as Array<{ duration?: number }>).reduce((sum, s) => sum + (s.duration || 5), 0),
      title: 'EP04 Part 2 — The Production',
    },
    closing: {
      duration: (EP04_PART2_STORYBOOK_BOOKENDS.closing as Array<{ duration?: number }>).reduce((sum, s) => sum + (s.duration || 5), 0),
